          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {(settings?.contactCards || []).map((info: any, index: number) => {
              ...
              return (
                ...
                >
                  {Icon ? <Icon className="w-12 h-12 mx-auto mb-4 text-[#F4C430]" /> : null}
                  <h3 className="text-xl mb-4">{info.title}</h3>
                  {info.details.map((detail: string) => (
                    <p key={detail} className="text-gray-600 text-sm mb-1">{detail}</p>
                  ))}
                </motion.div>
              );
            })}
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <h2 className="text-4xl mb-6">{page?.content?.map?.title || 'Find Us'}</h2>
